...
VERSION :	1.0
ROLE :		Implémentation de la classe CDlg1237S
***************************************************************************	*/
#include "stdafx.h"
#include "sicomex.h"
#include "Dlg1237S.h"
...
CDlg1237S::CDlg1237S(CWnd* pParent /*=NULL*/)
	: CDlgEquip(CDlg1237S::IDD, pParent)
{
	//{{AFX_DATA_INIT(CDlg1237S)
	m_frequence = _T("");
	m_err_retard = _T("");
	m_canal = _T("");
	m_code_incident = _T("");
...
void CDlg1237S::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CDlg1237S)
...
BEGIN_MESSAGE_MAP(CDlg1237S, CDialog)
	//{{AFX_MSG_MAP(CDlg1237S)
	ON_WM_TIMER()
...
/////////////////////////////////////////////////////////////////////////////
// CDlg1237S message handlers

void CDlg1237S::OnAide()
{
	int			iResult;
	CDlgAide	win;
...
// **************
	...
	m_spin.SubclassDlgItem(F8_SPIN,this);	
...
void CDlg1237S::GenerateurAuto()
{
	srand( (unsigned)time( NULL ) );

	if(((CEquip1237S*)equip)->AutoNiveau())
		((CEquip1237S*)equip)->ChangeNiveau(rand()%0xFF,FALSE);
}
// OnTimer : Rafraichissement de l'écran de controle **************************
// ****************************************************************************
void CDlg1237S::OnTimer(UINT nIDEvent) 
{
	int			val;
	char		buf[10];
...
// Verrouille((((CEquip1237S*)equip)->Local())!=1);

// Mode SPIN
		m_spin.Positionne_etat(((CEquip1237S*)equip)->Spin());

// Couplage
	SendDlgItemMessage(F8_COUPLAGE_MAITRE,BM_SETCHECK,0,0);
	SendDlgItemMessage(F8_COUPLAGE_ESCLAVE,BM_SETCHECK,0,0);
	val = ((CEquip1237S*)equip)->Couplage();
	switch(val)
	{
		case 0:
...
// Mode Trafic
	val = ((CEquip1237S*)equip)->ModeTrafic(0);
	m_trafic.SetCurSel(val);

// Compresseur
	val = ((CEquip1237S*)equip)->Compresseur();
	SendDlgItemMessage(F3_COMPRESSEUR,BM_SETCHECK,val,0);

// Mode de fonctionnement
	val = ((CEquip1237S*)equip)->Fonction(0);
	m_fonctionnement.SetCurSel(val);

// Incident
	val = ((CEquip1237S*)equip)->Incident();
...
// Frequence
	...
	{
		sprintf(buf,"%d",((CEquip1237S*)equip)->Frequence(0));
		m_frequence = buf;
	}

// Niveau analogique
	val = ((CEquip1237S*)equip)->AutoNiveau();
	SendDlgItemMessage(F3_AUTO,BM_SETCHECK,val,0);

	if(GetDlgItem(F3_NIVEAU) != GetFocus())
	{
		sprintf(buf,"%d",((CEquip1237S*)equip)->Niveau());
		m_niveau = buf;
	}

	val = ((CEquip1237S*)equip)->ChoixNiveau();
	m_choix_niveau.SetCurSel(val);

// Canal
	if(GetDlgItem(F3_CANAL) != GetFocus())
	{
		sprintf(buf,"%d",((CEquip1237S*)equip)->Canal());
		m_canal = buf;
	}
...
// Test
	...
	{
		sprintf(buf,"%d",((CEquip1237S*)equip)->Test());
		m_test = buf;
	}

// Presence Info
		m_info.Positionne_etat(((CEquip1237S*)equip)->Info());
...
// ******************************************************************************
void CDlg1237S::Verrouille(BOOL etat)
{
...
void CDlg1237S::OnSelchangeModeTrafic() 
{
	int	iResult = ((CEquip1237S*)equip)->ChangeModeTrafic(0,m_trafic.GetCurSel(),((CEquip1237S*)equip)->Local() != 0);
	if(iResult<0);	// L'ancienne valeur sera rafraichie
}

void CDlg1237S::OnModeDbl() 
{
	int	iResult = ((CEquip1237S*)equip)->ChangeLocal(2,TRUE);
	if(iResult<0);	// L'ancienne valeur sera rafraichie
}

void CDlg1237S::OnModeLoc() 
{
	int	iResult = ((CEquip1237S*)equip)->ChangeLocal(0,TRUE);
	if(iResult<0);	// L'ancienne valeur sera rafraichie
}

void CDlg1237S::OnModeRmt() 
{
	int	iResult = ((CEquip1237S*)equip)->ChangeLocal(1,TRUE);
	if(iResult<0);	// L'ancienne valeur sera rafraichie
}


void CDlg1237S::OnCompresseur() 
{
	int		iResult;

	int		val=((CButton*)GetDlgItem(F3_COMPRESSEUR))->GetCheck();
	iResult = ((CEquip1237S*)equip)->ChangeCompresseur(val,((CEquip1237S*)equip)->Local() != 0);
	if(iResult<0);	// L'ancienne valeur sera rafraichie
}

void CDlg1237S::OnSelchangeModeFonct() 
{
	int	iResult = ((CEquip1237S*)equip)->ChangeFonction(0,m_fonctionnement.GetCurSel(),((CEquip1237S*)equip)->Local() != 0);
	if(iResult<0);	// L'ancienne valeur sera rafraichie
}

void CDlg1237S::OnKillfocusCodeIncident() 
{
	int		iResult;

	UpdateData(TRUE);
	iResult = ((CEquip1237S*)equip)->ChangeIncident(atoi(m_code_incident),((CEquip1237S*)equip)->Local() != 0);
//	if(iResult<0);	// L'ancienne valeur sera rafraichie
}

void CDlg1237S::OnKillfocusFrequence() 
{
	long	lResult;

	UpdateData(TRUE);
	lResult = ((CEquip1237S*)equip)->ChangeFrequence(0,atoi(m_frequence),((CEquip1237S*)equip)->Local() != 0);
	if(lResult<0);	// L'ancienne valeur sera rafraichie
}

void CDlg1237S::OnAuto() 
{
	int		iResult;
	...
	if(m_choix_niveau.GetCurSel() != 0)
	{
		...
		iResult = ((CEquip1237S*)equip)->ChangeAutoNiveau(val);
		if(iResult<0);	// L'ancienne valeur sera rafraichie
	}
	else
	{
		SendDlgItemMessage(F3_AUTO,BST_UNCHECKED,0,0);
		((CEquip1237S*)equip)->ChangeAutoNiveau(0);
	}
}

void CDlg1237S::OnKillfocusNiveau() 
{
	int		iResult;

	UpdateData(TRUE);
	iResult = ((CEquip1237S*)equip)->ChangeNiveau(atoi(m_niveau),((CEquip1237S*)equip)->Local() != 0);
//	if(iResult<0);	// L'ancienne valeur sera rafraichie
}

void CDlg1237S::OnSelchangeChoixNiveau() 
{
	int	iResult = ((CEquip1237S*)equip)->ChangeChoixNiveau(m_choix_niveau.GetCurSel(),((CEquip1237S*)equip)->Local() != 0);
	if(iResult<0);	// L'ancienne valeur sera rafraichie
	/* Correction du 30/01/2008 */
	if(iResult == 0)
	{
...
void CDlg1237S::OnKillfocusCanal() 
{
	int		iResult;

	UpdateData(TRUE);
	iResult = ((CEquip1237S*)equip)->ChangeCanal(atoi(m_canal),((CEquip1237S*)equip)->Local() != 0);
//	if(iResult<0);	// L'ancienne valeur sera rafraichie
}

void CDlg1237S::OnMemoCanal() 
{
	int		iResult;

	int		num_canal = ((CEquip1237S*)equip)->Canal();

	iResult = ((CEquip1237S*)equip)->MemoriseCanal(num_canal,((CEquip1237S*)equip)->Local() != 0);
//	if(iResult<0);	// L'ancienne valeur sera rafraichie
}

void CDlg1237S::OnKillfocusCodeTest() 
{
	int		iResult;

	UpdateData(TRUE);
	iResult = ((CEquip1237S*)equip)->ChangeTest(atoi(m_test),((CEquip1237S*)equip)->Local() != 0);
//	if(iResult<0);	// L'ancienne valeur sera rafraichie
}

void CDlg1237S::OnInfo() 
{
	int	iResult = m_info.Inverse_etat();
 	((CEquip1237*)equip)->ChangeInfo(iResult,TRUE);
}

void CDlg1237S::OnSpin() 
{
	int	iResult = m_spin.Inverse_etat();
 	((CEquip1237S*)equip)->ChangeSpin(iResult,TRUE);
}


void CDlg1237S::OnCouplageEsclave() 
{
	int	iResult = ((CEquip1237*)equip)->ChangeCouplage(1,TRUE);
	if(iResult<0);	// L'ancienne valeur sera rafraichie
}

void CDlg1237S::OnCouplageMaitre() 
{
