/* **************************************************************************
FICHIER :	P_1236.CPP		 			AUTEUR : PhB
DATE DE CREATION : 	13/12/96			DERNIERE MODIFICATION : 14/02/97
VERSION :	1.0
ROLE :		Implémentation de la classe CProto1236
***************************************************************************	*/
...
***************************************************************************	*/
CProto1236::CProto1236(t_format fmt,CEquip *pEqp):CProto(fmt,pEqp)
{
	hexa = FALSE;
	lecture_canal = 0;
}
...
***************************************************************************	*/
int CProto1236::EnvoyerTS(char *message)
{
	int		len_mes,iResult;
...
TRAITEMENT:		Traite une TC (partie utile) et formate le message TS reponse
				comme acquitement
***************************************************************************	*/
int CProto1236::TraiteTC(char *mes)
{
	int		iResult = 0;
	int		valeur;
	long	long_valeur;
...
//		EnvoyerTS(reponse);
	...
	{
		iResult = ((CEquip1236*)eqp)->ChangeLocal(0,FALSE);
		strcpy(reponse,"loc");
		EnvoyerTS(reponse);
		return 0;
	}
	...
	{
		valeur = atoi(mes+1);
		iResult = ((CEquip1236*)eqp)->ChangeLocal(2,FALSE);
	...
	{
		iResult = ((CEquip1236*)eqp)->ChangeLocal(1,FALSE);
		
		strcpy(reponse,"rmt");
		EnvoyerTS(reponse);

		AjouterTS(NULL,EME_1236_OPTION);
		AjouterTS(NULL,EME_1236_FREQUENCE);
		AjouterTS(NULL,EME_1236_MODE_FONCT);
		AjouterTS(NULL,EME_1236_COMPRESSEUR);
		AjouterTS(NULL,EME_1236_MODE_TRAFIC);
		AjouterTS(NULL,EME_1236_CANAL_POSI);
		AjouterTS(NULL,EME_1236_INFO);
		return 0;
	}
	else
		if(((CEquip1236*)eqp)->Local() == 0) return ERR_PAS_ACQUITTE;

	switch(mes[0])
	{
		case 'M':
			valeur = atoi(mes+1);
			iResult = ((CEquip1236*)eqp)->ChangeModeTrafic(0,valeur,FALSE);
		...
		case 'Z':
			valeur = atoi(mes+1);
			iResult = ((CEquip1236*)eqp)->ChangeCompresseur(valeur-1,FALSE);
		...
		case 'B':
			...
			iResult = ((CEquip1236*)eqp)->ChangeFonction(0,valeur,FALSE);
			if(iResult>=0 && !((CEquip1236*)eqp)->Incident())		// Modification acceptée
				AjouterTS(NULL,EME_1236_MODE_FONCT);
			else
			{
				sprintf(reponse,"B%d=",valeur);
				AjouterTS(reponse,EME_1236_MODE_FONCT);
			}
			break;
		...
		case 'F':
			...
			lResult = ((CEquip1236*)eqp)->ChangeFrequence(0,long_valeur,FALSE);
  
			if(lResult >= 0)				// Modification acceptée Autre TS
			{
				ChangeCanalLecture(0);

				TraiteTS(EME_1236_FREQUENCE,reponse);
				reponse[0]='f';
				...
				reponse[0]='F';
				reponse[(int)strlen(reponse)-1] = '?';
				AjouterTS(reponse,EME_1236_FREQUENCE);

				AjouterTS(NULL,CDE_TEMPO_DIV_4);

				if(((CEquip1236*)eqp)->Incident())
				{
					reponse[(int)strlen(reponse)-1] = '=';
					AjouterTS(reponse,EME_1236_FREQUENCE);
				}
		...
		case 'H':
			valeur = atoi(mes+1);
			iResult = ((CEquip1236*)eqp)->ChangeChoixNiveau(valeur,FALSE);
			...
			if(iResult >= 0)				// Modification acceptée
			{
				...
				AjouterTS(NULL,EME_1236_NIVEAU);
			}
			if(iResult == 0)
			{
				((CEquip1236*)eqp)->ChangeAutoNiveau(0);
			}
			break;
		...
		case 'P':
			if(mes[1] == '#' || mes[1] == '=')
			{
				TraiteTS(EME_1236_CANAL_POSI,reponse);
				reponse[0]='p';
				reponse[(int)strlen(reponse)-1] = '>';
				EnvoyerTS(reponse);
			}
			...
			{
				valeur = atoi(mes+1);
				iResult = ((CEquip1236*)eqp)->ChangeCanal(valeur,FALSE);
				if(iResult >= 0)				// Modification acceptée Autre TS
				{
					...
					reponse[0]='P';
					reponse[(int)strlen(reponse)-1] = '?';
					AjouterTS(reponse,EME_1236_CANAL_POSI);
		...
		case 'C':
			...
			if((int)strlen(mes+1)>5)		// canal,M,E,B : Modif canal
			{
				...
				iResult = ((CEquip1236*)eqp)->ChangeFonction(CanalLecture(),valeur,FALSE);
				...
				// Mode de trafic
				valeur = atoi(reponse+(int)strlen(reponse)-1);
				iResult = ((CEquip1236*)eqp)->ChangeModeTrafic(CanalLecture(),valeur,FALSE);
				reponse[(int)strlen(reponse)-1] = 0;
	
				// frequence
				long_valeur = atol(reponse) * Puissance(10,8-(int)strlen(reponse));
				lResult = ((CEquip1236*)eqp)->ChangeFrequence(CanalLecture(),long_valeur,FALSE);
			}

			if(iResult >= 0)				// Modification acceptée Autre TS
			{
				TraiteTS(EME_1236_CANAL_LECT,reponse);
				reponse[0]='c';
				EnvoyerTS(reponse);
			}
			break;
...
TRAITEMENT:		Formate une TS en fonction en réccupérant les etats internes
				de l'equipement
***************************************************************************	*/
int CProto1236:: TraiteTS(int type_cde,char *reponse)
{
	int		iResult;
	long	lResult;
	...
	switch(type_cde)
	{
		case EME_1236_MODE_LOCAL:
			iResult = ((CEquip1236*)eqp)->Local();
			switch(iResult)
			{
				case 1:
					strcpy(reponse,"RMT");
		...
		case EME_1236_MODE_TRAFIC:
			iResult = ((CEquip1236*)eqp)->ModeTrafic(CanalLecture());
			sprintf(reponse,"M%d",iResult);
			break;

		case EME_1236_COMPRESSEUR:
			iResult = ((CEquip1236*)eqp)->Compresseur();
			sprintf(reponse,"Z%d",iResult+1);
			break;

		case EME_1236_MODE_FONCT:
			iResult = ((CEquip1236*)eqp)->Fonction(CanalLecture());
			sprintf(reponse,"B%d>",iResult);
			break;

		case EME_1236_INCIDENT:
			iResult = ((CEquip1236*)eqp)->Incident();
			if(iResult == -1)
			{
				strcpy(reponse,"J=");
				iResult = 0;
		...
		case EME_1236_FREQUENCE:
			lResult = ((CEquip1236*)eqp)->Frequence(CanalLecture());
		...
		case EME_1236_NIVEAU:
			iResult = ((CEquip1236*)eqp)->ChoixNiveau();
			switch(iResult)
			{
				case 1:
				case 2:
		...
		case EME_1236_CANAL_POSI:
			iResult = ((CEquip1236*)eqp)->Canal();
			sprintf(reponse,"P%2.2d",iResult);

			TraiteTS(EME_1236_FREQUENCE,buf);
			buf[(int)strlen(buf)-1]=0; // Pas de caractere de positionnement
			strcat(reponse,buf+1); 


			TraiteTS(EME_1236_MODE_TRAFIC,buf);
			strcat(reponse,buf+1);

			// Caractère réservé
			strcat(reponse,"0");

			TraiteTS(EME_1236_MODE_FONCT,buf);
			buf[(int)strlen(buf)-1]=0; // Pas de caractere de positionnement
			strcat(reponse,buf+1);
		...
		case EME_1236_OPTION:
			sprintf(reponse,"OP78",iResult);
			break;

		case EME_1236_TEST:
			iResult = ((CEquip1236*)eqp)->Test();
			sprintf(reponse,"TST%2.2d",iResult);
			break;

		case EME_1236_ETAT_LOGI:
			strcpy(reponse,"XX_");
			break;

		case EME_1236_DATA_OUT:
			TraiteTS(EME_1236_CANAL_POSI,reponse);
			reponse[0]='Y';
...
TRAITEMENT:		Réceptionne une TC, Valide le bon format et en extrait la partie
				utile du message
***************************************************************************	*/
int CProto1236::RecevoirTC(char *message,int nb_car)
{
	char	buf[TAILLE_BUFFER+1];
	int		i,iResult;
...
***************************************************************************	*/
BOOL CProto1236::ValideAcquittement(int type_cde,char *buf)
{
	int		iResult = NULL;
	BOOL	bResult;
...
***************************************************************************	*/
char* CProto1236::ControleTrame(char *message,char *octet_controle)
{
	int		i,len_mes;
	char	pc;
...
TRAITEMENT:		Génère les octets de controle du message
				Un octet est necessaire par tranche de 8 caractere
***************************************************************************	*/
BOOL CProto1236::ExtraitUtile(char *buf,char *message,int *long_utile)
{
	char 	octet_controle[8];
...
***************************************************************************	*/
BOOL CProto1236::ChangeCanalLecture(int	valeur)
{
