...
VERSION :	1.0
ROLE :		Implémentation de la classe CDlg1241S
***************************************************************************	*/
...
CDlg1241S::CDlg1241S(CWnd* pParent /*=NULL*/)
	: CDlgEquip(CDlg1241S::IDD, pParent)
{
	//{{AFX_DATA_INIT(CDlg1241S)
	m_err_retard = _T("");
	m_bfo = _T("");
	m_canal = _T("");
	m_centrage = _T("");
...
void CDlg1241S::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CDlg1241S)
...
BEGIN_MESSAGE_MAP(CDlg1241S, CDialog)
	//{{AFX_MSG_MAP(CDlg1241S)
...
// **************
	...
	m_spin.SubclassDlgItem(F5_SPIN,this);	
...
void CDlg1241S::GenerateurAuto()
{
	srand( (unsigned)time( NULL ) );

	if(((CEquip1241S*)equip)->AutoMesureur())
		((CEquip1241S*)equip)->ChangeMesureur(0,rand()%109,FALSE);

	if(((CEquip1241S*)equip)->AutoCentrageF1())
		((CEquip1241S*)equip)->ChangeCentrageF1(rand()%077,FALSE);
}

// OnTimer : Rafraichissement de l'écran de controle **************************
// ****************************************************************************
void CDlg1241S::OnTimer(UINT nIDEvent) 
{
	int		val;
	char	buf[10];
...
// Verrouille(!((CEquip1241S*)equip)->Local());

// Mode SPIN
		m_spin.Positionne_etat(((CEquip1241S*)equip)->Spin());

// Mode Local
	SendDlgItemMessage(F4_MODE_LOC,BM_SETCHECK,0,0);
	SendDlgItemMessage(F4_MODE_RMT,BM_SETCHECK,0,0);
	val = ((CEquip1241S*)equip)->Local();
	if(val)
...
// Mode reception
	val = ((CEquip1241S*)equip)->ModeRecep(0);
...
// Attenuateur HF
	...
	val = ((CEquip1241S*)equip)->Attenuateur(0);
	switch(val)
	{
		case 0:
...
// Filtre FI
	val = ((CEquip1241S*)equip)->FiltreFI(0);
	if(val>4)
...
// CAG
	val = ((CEquip1241S*)equip)->Cag(0);
	m_cag.SetCurSel(val);
	if(val==4)
		SendDlgItemMessage(F4_BP4,BM_SETCHECK,1,0);

// Squelch
	val = ((CEquip1241S*)equip)->Squelch(0);
	SendDlgItemMessage(F4_SQUELCH,BM_SETCHECK,val,0);

// Sens de manipulation télégraphique
	val = ((CEquip1241S*)equip)->SensManip(0);
	m_sens_manip.SetCurSel(val);

// Shift
	val = ((CEquip1241S*)equip)->Shift(0);
	m_shift.SetCurSel(val);

// Frequence
	if(GetDlgItem(F4_FREQUENCE) != GetFocus())
	{
		sprintf(buf,"%d",((CEquip1241S*)equip)->Frequence(0));
		m_frequence = buf;
	}
...
// Frequence BFO
	...
	{
		sprintf(buf,"%d",((CEquip1241S*)equip)->Bfo(0));
		m_bfo = buf;
	}

// Avec Rejecteur
	val = ((CEquip1241S*)equip)->AvecRejecteur();
...
// Frequence Rejecteur
	...
	{
		sprintf(buf,"%d",((CEquip1241S*)equip)->Rejecteur());
...
// Gain HF
	...
	{
		sprintf(buf,"%d",((CEquip1241S*)equip)->GainHF(0));
		m_gain_hf = buf;
	}
...
// Mesureur de champs
	...
	{
		sprintf(buf,"%d",((CEquip1241S*)equip)->Mesureur(0));
		m_mesureur = buf;
	}

	val = ((CEquip1241S*)equip)->AutoMesureur();
	SendDlgItemMessage(F4_AUTO_MESUREUR,BM_SETCHECK,val,0);

// Seuil Cor/Squelch
	if(GetDlgItem(F4_COR_SQUELCH) != GetFocus())
	{
		sprintf(buf,"%d",((CEquip1241S*)equip)->CorSquelch());
		m_cor_squelch = buf;
	}
...
// Centrage Démodulateur F1
	...
	{
		sprintf(buf,"%d",((CEquip1241S*)equip)->CentrageF1());
		m_centrage = buf;
	}

	val = ((CEquip1241S*)equip)->AutoCentrageF1();
	SendDlgItemMessage(F4_AUTO_CENTRAGE,BM_SETCHECK,val,0);

// Canal
	if(GetDlgItem(F4_CANAL) != GetFocus())
	{
		sprintf(buf,"%d",((CEquip1241S*)equip)->Canal());
		m_canal = buf;
	}
...
// Code Test
	...
	{
		sprintf(buf,"%d",((CEquip1241S*)equip)->Test());
		m_code_test = buf;
	}

// Presence Infos
	val = ((CEquip1241S*)equip)->Info();
...
void CDlg1241S::OnSelchangeModeReception() 
{
	int	iResult = ((CEquip1241S*)equip)->ChangeModeRecep(0,m_mode_reception.GetCurSel(),((CEquip1241S*)equip)->Local() != 0);
	if(iResult<0);	// L'ancienne valeur sera rafraichie
}

void CDlg1241S::OnIdcAttenuateur0() 
{
	int	iResult = ((CEquip1241S*)equip)->ChangeAttenuateur(0,0,((CEquip1241S*)equip)->Local() != 0);
	if(iResult<0);	// L'ancienne valeur sera rafraichie
}

void CDlg1241S::OnIdcAttenuateur1() 
{
	int	iResult = ((CEquip1241S*)equip)->ChangeAttenuateur(0,1,((CEquip1241S*)equip)->Local() != 0);
	if(iResult<0);	// L'ancienne valeur sera rafraichie
}

void CDlg1241S::OnIdcAttenuateur2() 
{
	int	iResult = ((CEquip1241S*)equip)->ChangeAttenuateur(0,2,((CEquip1241S*)equip)->Local() != 0);
	if(iResult<0);	// L'ancienne valeur sera rafraichie
}


void CDlg1241S::OnBp1() 
{
	int		val;
	...
	int	iResult = ((CEquip1241S*)equip)->ChangeFiltreFI(0,val,((CEquip1241S*)equip)->Local() != 0);
	if(iResult<0);	// L'ancienne valeur sera rafraichie
}

void CDlg1241S::OnBp2() 
{
	int		val;
...
void CDlg1241S::OnSelchangeCag() 
{
	int	iResult = ((CEquip1241S*)equip)->ChangeCag(0,m_cag.GetCurSel(),((CEquip1241S*)equip)->Local() != 0);
	if(iResult<0);	// L'ancienne valeur sera rafraichie
}

void CDlg1241S::OnSquelch() 
{
	int		iResult;

	int		val=((CButton*)GetDlgItem(F4_SQUELCH))->GetCheck();
	iResult = ((CEquip1241S*)equip)->ChangeSquelch(0,val,((CEquip1241S*)equip)->Local() != 0);
	if(iResult<0);	// L'ancienne valeur sera rafraichie
}

void CDlg1241S::OnSelchangeInvTg() 
{
	
	int	iResult = ((CEquip1241S*)equip)->ChangeSensManip(0,m_sens_manip.GetCurSel(),((CEquip1241S*)equip)->Local() != 0);
	if(iResult<0);	// L'ancienne valeur sera rafraichie
}


void CDlg1241S::OnSelchangeShift() 
{
	int	iResult = ((CEquip1241S*)equip)->ChangeShift(0,m_shift.GetCurSel(),((CEquip1241S*)equip)->Local() != 0);
	if(iResult<0);	// L'ancienne valeur sera rafraichie
}

void CDlg1241S::OnKillfocusFrequence() 
{
	long	lResult;

	UpdateData(TRUE);
	lResult = ((CEquip1241S*)equip)->ChangeFrequence(0,atoi(m_frequence),((CEquip1241S*)equip)->Local() != 0);
	if(lResult<0);	// L'ancienne valeur sera rafraichie
}

void CDlg1241S::OnKillfocusBfo() 
{
	int		iResult;

	UpdateData(TRUE);
	iResult = ((CEquip1241S*)equip)->ChangeBfo(0,atoi(m_bfo),((CEquip1241S*)equip)->Local() != 0);
//	if(iResult<0);	// L'ancienne valeur sera rafraichie
}

void CDlg1241S::OnRejecteur() 
{
	int		iResult;

	int		val=((CButton*)GetDlgItem(F4_REJECTEUR))->GetCheck();
	iResult = ((CEquip1241S*)equip)->ChangeAvecRejecteur(val,((CEquip1241S*)equip)->Local() != 0);
	if(iResult<0);	// L'ancienne valeur sera rafraichie
}

void CDlg1241S::OnKillfocusFreqReject() 
{
	int		iResult;
	UpdateData(TRUE);
	iResult = ((CEquip1241S*)equip)->ChangeRejecteur(atoi(m_freq_rejecteur),((CEquip1241S*)equip)->Local() != 0);
//	if(iResult<0);	// L'ancienne valeur sera rafraichie
}

void CDlg1241S::OnKillfocusGainHf() 
{
	int		iResult;

	UpdateData(TRUE);
	iResult = ((CEquip1241S*)equip)->ChangeGainHF(0,atoi(m_gain_hf),((CEquip1241S*)equip)->Local() != 0);
//	if(iResult<0);	// L'ancienne valeur sera rafraichie
}

void CDlg1241S::OnAutoMesureur() 
{
	int		iResult;

	int		val=((CButton*)GetDlgItem(F4_AUTO_MESUREUR))->GetCheck();
	iResult = ((CEquip1241S*)equip)->ChangeAutoMesureur(val);
	if(iResult<0);	// L'ancienne valeur sera rafraichie
}

void CDlg1241S::OnKillfocusMesureur() 
{
	int		iResult;

	UpdateData(TRUE);
	iResult = ((CEquip1241S*)equip)->ChangeMesureur(0,atoi(m_mesureur),((CEquip1241S*)equip)->Local() != 0);
//	if(iResult<0);	// L'ancienne valeur sera rafraichie
}

void CDlg1241S::OnKillfocusCorSquelch() 
{
	int		iResult;

	UpdateData(TRUE);
	iResult = ((CEquip1241S*)equip)->ChangeCorSquelch(atoi(m_cor_squelch),((CEquip1241S*)equip)->Local() != 0);
//	if(iResult<0);	// L'ancienne valeur sera rafraichie
}

void CDlg1241S::OnAutoCentrage() 
{
	int		iResult;

	int		val=((CButton*)GetDlgItem(F4_AUTO_CENTRAGE))->GetCheck();
	iResult = ((CEquip1241S*)equip)->ChangeAutoCentrageF1(val);
	if(iResult<0);	// L'ancienne valeur sera rafraichie
}

void CDlg1241S::OnKillfocusCentrage() 
{
	int		iResult;

	UpdateData(TRUE);
	iResult = ((CEquip1241S*)equip)->ChangeCentrageF1(atoi(m_centrage),((CEquip1241S*)equip)->Local() != 0);
//	if(iResult<0);	// L'ancienne valeur sera rafraichie
}

void CDlg1241S::OnKillfocusCanal() 
{
	int		iResult;

	UpdateData(TRUE);
	iResult = ((CEquip1241S*)equip)->ChangeCanal(atoi(m_canal),((CEquip1241S*)equip)->Local() != 0);
//	if(iResult<0);	// L'ancienne valeur sera rafraichie
	
}

void CDlg1241S::OnMemoCanal() 
{
	int		iResult;

	int		num_canal = ((CEquip1241S*)equip)->Canal();

	iResult = ((CEquip1241S*)equip)->MemoriseCanal(num_canal,((CEquip1241S*)equip)->Local() != 0);
//	if(iResult<0);	// L'ancienne valeur sera rafraichie
}

void CDlg1241S::OnKillfocusCodeTest() 
{
	int		iResult;

	UpdateData(TRUE);
	iResult = ((CEquip1241S*)equip)->ChangeTest(atoi(m_code_test),((CEquip1241S*)equip)->Local() != 0);
//	if(iResult<0);	// L'ancienne valeur sera rafraichie
}

void CDlg1241S::OnModeLoc() 
{
	int	iResult = ((CEquip1241S*)equip)->ChangeLocal(0,TRUE);
	if(iResult<0);	// L'ancienne valeur sera rafraichie

}

void CDlg1241S::OnModeRmt() 
{
	int	iResult = ((CEquip1241S*)equip)->ChangeLocal(1,TRUE);
	if(iResult<0);	// L'ancienne valeur sera rafraichie

}

void CDlg1241S::Verrouille(BOOL etat)
{
...
void CDlg1241S::OnSpin() 
{
	int	iResult = m_spin.Inverse_etat();
 	((CEquip1241*)equip)->ChangeSpin(iResult,TRUE);
}
