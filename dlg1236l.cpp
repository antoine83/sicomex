/* **************************************************************************
FICHIER :	Dlg1236L.CPP		 		AUTEUR : PhB
DATE DE CREATION : 	13/12/96			DERNIERE MODIFICATION : 14/02/97
VERSION :	1.0
ROLE :		Implémentation de la classe CDlg1236LL
***************************************************************************	*/
...
CDlg1236L::CDlg1236L(CWnd* pParent /*=NULL*/)
	: CDlgEquip(CDlg1236L::IDD, pParent)
{
	//{{AFX_DATA_INIT(CDlg1236L)
	m_frequence = _T("");
	m_err_retard = _T("");
	m_canal = _T("");
	m_code_incident = _T("");
...
void CDlg1236L::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CDlg1236L)
...
BEGIN_MESSAGE_MAP(CDlg1236L, CDialog)
	//{{AFX_MSG_MAP(CDlg1236L)
	ON_WM_TIMER()
...
/////////////////////////////////////////////////////////////////////////////
// CDlg1236L message handlers

void CDlg1236L::OnAide()
{
	int			iResult;
	CDlgAide	win;
...
void CDlg1236L::GenerateurAuto()
{
	srand( (unsigned)time( NULL ) );

	if(((CEquip1236L*)equip)->AutoNiveau())
		((CEquip1236L*)equip)->ChangeNiveau(rand()%0xFF,FALSE);
}
// OnTimer : Rafraichissement de l'écran de controle **************************
// ****************************************************************************
void CDlg1236L::OnTimer(UINT nIDEvent) 
{
	int			val;
	char		buf[10];
...
// Test de changement dans la face avant
	...
	// Verrouille((((CEquip1236L*)equip)->Local())!=1);
...
// Mode Trafic
	val = ((CEquip1236L*)equip)->ModeTrafic(0);
	m_trafic.SetCurSel(val);

// Compresseur
	val = ((CEquip1236L*)equip)->Compresseur();
	SendDlgItemMessage(F3_COMPRESSEUR,BM_SETCHECK,val,0);

// Mode de fonctionnement
	val = ((CEquip1236L*)equip)->Fonction(0);
	m_fonctionnement.SetCurSel(val);

// Incident
	val = ((CEquip1236L*)equip)->Incident();
...
// Frequence
	...
	{
		sprintf(buf,"%d",((CEquip1236L*)equip)->Frequence(0));
		m_frequence = buf;
	}

// Niveau analogique
	val = ((CEquip1236L*)equip)->AutoNiveau();
	SendDlgItemMessage(F3_AUTO,BM_SETCHECK,val,0);

	if(GetDlgItem(F3_NIVEAU) != GetFocus())
	{
		sprintf(buf,"%d",((CEquip1236L*)equip)->Niveau());
		m_niveau = buf;
	}

	val = ((CEquip1236L*)equip)->ChoixNiveau();
	m_choix_niveau.SetCurSel(val);

// Canal
	if(GetDlgItem(F3_CANAL) != GetFocus())
	{
		sprintf(buf,"%d",((CEquip1236L*)equip)->Canal());
		m_canal = buf;
	}
...
// Test
	...
	{
		sprintf(buf,"%d",((CEquip1236L*)equip)->Test());
		m_test = buf;
	}

// Presence Info
		m_info.Positionne_etat(((CEquip1236L*)equip)->Info());
...
// ******************************************************************************
void CDlg1236L::Verrouille(BOOL etat)
{
...
void CDlg1236L::OnSelchangeModeTrafic() 
{
	int	iResult = ((CEquip1236L*)equip)->ChangeModeTrafic(0,m_trafic.GetCurSel(),((CEquip1236L*)equip)->Local() != 0);
	if(iResult<0);	// L'ancienne valeur sera rafraichie
}

void CDlg1236L::OnModeDbl() 
{
	int	iResult = ((CEquip1236L*)equip)->ChangeLocal(2,TRUE);
	if(iResult<0);	// L'ancienne valeur sera rafraichie
}

void CDlg1236L::OnModeLoc() 
{
	int	iResult = ((CEquip1236L*)equip)->ChangeLocal(0,TRUE);
	if(iResult<0);	// L'ancienne valeur sera rafraichie
}

void CDlg1236L::OnModeRmt() 
{
	int	iResult = ((CEquip1236L*)equip)->ChangeLocal(1,TRUE);
	if(iResult<0);	// L'ancienne valeur sera rafraichie
}


void CDlg1236L::OnCompresseur() 
{
	int		iResult;

	int		val=((CButton*)GetDlgItem(F3_COMPRESSEUR))->GetCheck();
	iResult = ((CEquip1236L*)equip)->ChangeCompresseur(val,((CEquip1236L*)equip)->Local() != 0);
	if(iResult<0);	// L'ancienne valeur sera rafraichie
}

void CDlg1236L::OnSelchangeModeFonct() 
{
	int	iResult = ((CEquip1236L*)equip)->ChangeFonction(0,m_fonctionnement.GetCurSel(),((CEquip1236L*)equip)->Local() != 0);
	if(iResult<0);	// L'ancienne valeur sera rafraichie
}

void CDlg1236L::OnKillfocusCodeIncident() 
{
	int		iResult;

	UpdateData(TRUE);
	iResult = ((CEquip1236L*)equip)->ChangeIncident(atoi(m_code_incident),((CEquip1236L*)equip)->Local() != 0);
//	if(iResult<0);	// L'ancienne valeur sera rafraichie
}

void CDlg1236L::OnKillfocusFrequence() 
{
	long	lResult;

	UpdateData(TRUE);
	lResult = ((CEquip1236L*)equip)->ChangeFrequence(0,atoi(m_frequence),((CEquip1236L*)equip)->Local() != 0);
	if(lResult<0);	// L'ancienne valeur sera rafraichie
}

void CDlg1236L::OnAuto() 
{
	int		iResult;
	...
	if(m_choix_niveau.GetCurSel() != 0)
	{
		...
		iResult = ((CEquip1236L*)equip)->ChangeAutoNiveau(val);
		if(iResult<0);	// L'ancienne valeur sera rafraichie
	}
	else
	{
		SendDlgItemMessage(F3_AUTO,BST_UNCHECKED,0,0);
		((CEquip1236L*)equip)->ChangeAutoNiveau(0);
	}
}

void CDlg1236L::OnKillfocusNiveau() 
{
	int		iResult;

	UpdateData(TRUE);
	iResult = ((CEquip1236L*)equip)->ChangeNiveau(atoi(m_niveau),((CEquip1236L*)equip)->Local() != 0);
//	if(iResult<0);	// L'ancienne valeur sera rafraichie
}

void CDlg1236L::OnSelchangeChoixNiveau() 
{
	int	iResult = ((CEquip1236L*)equip)->ChangeChoixNiveau(m_choix_niveau.GetCurSel(),((CEquip1236L*)equip)->Local() != 0);
	if(iResult<0);	// L'ancienne valeur sera rafraichie
	/* Correction du 30/01/2008 */
	if(iResult == 0)
	{
...
void CDlg1236L::OnKillfocusCanal() 
{
	int		iResult;

	UpdateData(TRUE);
	iResult = ((CEquip1236L*)equip)->ChangeCanal(atoi(m_canal),((CEquip1236L*)equip)->Local() != 0);
//	if(iResult<0);	// L'ancienne valeur sera rafraichie
}

void CDlg1236L::OnMemoCanal() 
{
	int		iResult;

	int		num_canal = ((CEquip1236L*)equip)->Canal();

	iResult = ((CEquip1236L*)equip)->MemoriseCanal(num_canal,((CEquip1236L*)equip)->Local() != 0);
//	if(iResult<0);	// L'ancienne valeur sera rafraichie
}

void CDlg1236L::OnKillfocusCodeTest() 
{
	int		iResult;

	UpdateData(TRUE);
	iResult = ((CEquip1236L*)equip)->ChangeTest(atoi(m_test),((CEquip1236L*)equip)->Local() != 0);
//	if(iResult<0);	// L'ancienne valeur sera rafraichie
}

void CDlg1236L::OnInfo() 
{
