/* **************************************************************************
FICHIER :	DlgEmHF.CPP		 		AUTEUR : PhB
DATE DE CREATION : 	13/12/96			DERNIERE MODIFICATION : 14/02/97
VERSION :	1.0
ROLE :		Implémentation de la classe CDlgEmetHF
***************************************************************************	*/
...
CDlgEmetHF::CDlgEmetHF(CWnd* pParent /*=NULL*/)
	: CDlgEquip(CDlgEmetHF::IDD, pParent)
{
	//{{AFX_DATA_INIT(CDlgEmetHF)
	m_frequence = _T("");
	m_err_retard = _T("");
	m_canal = _T("");
	m_code_incident = _T("");
...
void CDlgEmetHF::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CDlgEmetHF)
...
BEGIN_MESSAGE_MAP(CDlgEmetHF, CDialog)
	//{{AFX_MSG_MAP(CDlgEmetHF)
	ON_WM_TIMER()
...
/////////////////////////////////////////////////////////////////////////////
// CDlgEmetHF message handlers

void CDlgEmetHF::OnAide()
{
	int			iResult;
	CDlgAide	win;
...
// ****************************
	...
	m_liste_message.AddString("Mode Fonction.");
...
void CDlgEmetHF::GenerateurAuto()
{
	srand( (unsigned)time( NULL ) );

	if(((CEquip1225*)equip)->AutoNiveau())
		((CEquip1225*)equip)->ChangeNiveau(rand()%0xFF,FALSE);
}
// OnTimer : Rafraichissement de l'écran de controle **************************
// ****************************************************************************
void CDlgEmetHF::OnTimer(UINT nIDEvent) 
{
	int			val;
	char		buf[10];
...
// Test de changement dans la face avant
	...
	// Verrouille((((CEquip1225*)equip)->Local())!=1);
...
// Mode Trafic
	val = ((CEquip1225*)equip)->ModeTrafic(0);
	m_trafic.SetCurSel(val);

// Compresseur
	val = ((CEquip1225*)equip)->Compresseur();
	SendDlgItemMessage(F3_COMPRESSEUR,BM_SETCHECK,val,0);

// Mode de fonctionnement
	val = ((CEquip1225*)equip)->Fonction(0);
	m_fonctionnement.SetCurSel(val);

// Incident
	val = ((CEquip1225*)equip)->Incident();
...
// Frequence
	...
	{
		sprintf(buf,"%d",((CEquip1225*)equip)->Frequence(0));
		m_frequence = buf;
	}

// Niveau analogique
	val = ((CEquip1225*)equip)->AutoNiveau();
	SendDlgItemMessage(F3_AUTO,BM_SETCHECK,val,0);

	if(GetDlgItem(F3_NIVEAU) != GetFocus())
	{
		sprintf(buf,"%d",((CEquip1225*)equip)->Niveau());
		m_niveau = buf;
	}

	val = ((CEquip1225*)equip)->ChoixNiveau();
	m_choix_niveau.SetCurSel(val);

// Canal
	if(GetDlgItem(F3_CANAL) != GetFocus())
	{
		sprintf(buf,"%d",((CEquip1225*)equip)->Canal());
		m_canal = buf;
	}
...
// Test
	...
	{
		sprintf(buf,"%d",((CEquip1225*)equip)->Test());
		m_test = buf;
	}

// Presence Info
		m_info.Positionne_etat(((CEquip1225*)equip)->Info());
...
// ******************************************************************************
void CDlgEmetHF::Verrouille(BOOL etat)
{
...
void CDlgEmetHF::OnSelchangeModeTrafic() 
{
	int	iResult = ((CEquip1225*)equip)->ChangeModeTrafic(0,m_trafic.GetCurSel(),((CEquip1225*)equip)->Local() != 0);
	if(iResult<0);	// L'ancienne valeur sera rafraichie
}

void CDlgEmetHF::OnModeDbl() 
{
	int	iResult = ((CEquip1225*)equip)->ChangeLocal(2,TRUE);
	if(iResult<0);	// L'ancienne valeur sera rafraichie
}

void CDlgEmetHF::OnModeLoc() 
{
	int	iResult = ((CEquip1225*)equip)->ChangeLocal(0,TRUE);
	if(iResult<0);	// L'ancienne valeur sera rafraichie
}

void CDlgEmetHF::OnModeRmt() 
{
	int	iResult = ((CEquip1225*)equip)->ChangeLocal(1,TRUE);
	if(iResult<0);	// L'ancienne valeur sera rafraichie
}


void CDlgEmetHF::OnCompresseur() 
{
	int		iResult;

	int		val=((CButton*)GetDlgItem(F3_COMPRESSEUR))->GetCheck();
	iResult = ((CEquip1225*)equip)->ChangeCompresseur(val,((CEquip1225*)equip)->Local() != 0);
	if(iResult<0);	// L'ancienne valeur sera rafraichie
}

void CDlgEmetHF::OnSelchangeModeFonct() 
{
	int	iResult = ((CEquip1225*)equip)->ChangeFonction(0,m_fonctionnement.GetCurSel(),((CEquip1225*)equip)->Local() != 0);
	if(iResult<0);	// L'ancienne valeur sera rafraichie
}

void CDlgEmetHF::OnKillfocusCodeIncident() 
{
	int		iResult;

	UpdateData(TRUE);
	iResult = ((CEquip1225*)equip)->ChangeIncident(atoi(m_code_incident),((CEquip1225*)equip)->Local() != 0);
//	if(iResult<0);	// L'ancienne valeur sera rafraichie
}

void CDlgEmetHF::OnKillfocusFrequence() 
{
	long	lResult;

	UpdateData(TRUE);
	lResult = ((CEquip1225*)equip)->ChangeFrequence(0,atoi(m_frequence),((CEquip1225*)equip)->Local() != 0);
	if(lResult<0);	// L'ancienne valeur sera rafraichie
}

void CDlgEmetHF::OnAuto() 
{
	int		iResult;

	int		val=((CButton*)GetDlgItem(F3_AUTO))->GetCheck();
	iResult = ((CEquip1225*)equip)->ChangeAutoNiveau(val);
	if(iResult<0);	// L'ancienne valeur sera rafraichie
}

void CDlgEmetHF::OnKillfocusNiveau() 
{
	int		iResult;

	UpdateData(TRUE);
	iResult = ((CEquip1225*)equip)->ChangeNiveau(atoi(m_niveau),((CEquip1225*)equip)->Local() != 0);
//	if(iResult<0);	// L'ancienne valeur sera rafraichie
}

void CDlgEmetHF::OnSelchangeChoixNiveau() 
{
	int	iResult = ((CEquip1225*)equip)->ChangeChoixNiveau(m_choix_niveau.GetCurSel(),((CEquip1225*)equip)->Local() != 0);
	if(iResult<0);	// L'ancienne valeur sera rafraichie
}

void CDlgEmetHF::OnKillfocusCanal() 
{
	int		iResult;

	UpdateData(TRUE);
	iResult = ((CEquip1225*)equip)->ChangeCanal(atoi(m_canal),((CEquip1225*)equip)->Local() != 0);
//	if(iResult<0);	// L'ancienne valeur sera rafraichie
}

void CDlgEmetHF::OnMemoCanal() 
{
	int		iResult;

	int		num_canal = ((CEquip1225*)equip)->Canal();

	iResult = ((CEquip1225*)equip)->MemoriseCanal(num_canal,((CEquip1225*)equip)->Local() != 0);
//	if(iResult<0);	// L'ancienne valeur sera rafraichie
}

void CDlgEmetHF::OnKillfocusCodeTest() 
{
	int		iResult;

	UpdateData(TRUE);
	iResult = ((CEquip1225*)equip)->ChangeTest(atoi(m_test),((CEquip1225*)equip)->Local() != 0);
//	if(iResult<0);	// L'ancienne valeur sera rafraichie
}
