...
VERSION :	1.0
ROLE :		Implémentation de la classe CDlg1236
***************************************************************************	*/
...
CDlg1236::CDlg1236(CWnd* pParent /*=NULL*/)
	: CDlgEquip(CDlg1236::IDD, pParent)
{
	//{{AFX_DATA_INIT(CDlg1236)
	m_frequence = _T("");
	m_err_retard = _T("");
	m_canal = _T("");
	m_code_incident = _T("");
...
void CDlg1236::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CDlg1236)
...
BEGIN_MESSAGE_MAP(CDlg1236, CDialog)
	//{{AFX_MSG_MAP(CDlg1236)
	ON_WM_TIMER()
...
/////////////////////////////////////////////////////////////////////////////
// CDlg1236 message handlers

void CDlg1236::OnAide()
{
	int			iResult;
	CDlgAide	win;
...
void CDlg1236::GenerateurAuto()
{
	srand( (unsigned)time( NULL ) );

	if(((CEquip1236*)equip)->AutoNiveau())
		((CEquip1236*)equip)->ChangeNiveau(rand()%0xFF,FALSE);
}
// OnTimer : Rafraichissement de l'écran de controle **************************
// ****************************************************************************
void CDlg1236::OnTimer(UINT nIDEvent) 
{
	int			val;
	char		buf[10];
...
// Test de changement dans la face avant
	...
	// Verrouille((((CEquip1236*)equip)->Local())!=1);
...
// Mode Trafic
	val = ((CEquip1236*)equip)->ModeTrafic(0);
	m_trafic.SetCurSel(val);

// Compresseur
	val = ((CEquip1236*)equip)->Compresseur();
	SendDlgItemMessage(F3_COMPRESSEUR,BM_SETCHECK,val,0);

// Mode de fonctionnement
	val = ((CEquip1236*)equip)->Fonction(0);
	m_fonctionnement.SetCurSel(val);

// Incident
	val = ((CEquip1236*)equip)->Incident();
...
// Frequence
	...
	{
		sprintf(buf,"%d",((CEquip1236*)equip)->Frequence(0));
		m_frequence = buf;
	}

// Niveau analogique
	val = ((CEquip1236*)equip)->AutoNiveau();
	SendDlgItemMessage(F3_AUTO,BM_SETCHECK,val,0);

	if(GetDlgItem(F3_NIVEAU) != GetFocus())
	{
		sprintf(buf,"%d",((CEquip1236*)equip)->Niveau());
		m_niveau = buf;
	}

	val = ((CEquip1236*)equip)->ChoixNiveau();
	m_choix_niveau.SetCurSel(val);

// Canal
	if(GetDlgItem(F3_CANAL) != GetFocus())
	{
		sprintf(buf,"%d",((CEquip1236*)equip)->Canal());
		m_canal = buf;
	}
...
// Test
	...
	{
		sprintf(buf,"%d",((CEquip1236*)equip)->Test());
		m_test = buf;
	}

// Presence Info
		m_info.Positionne_etat(((CEquip1236*)equip)->Info());
...
// ******************************************************************************
void CDlg1236::Verrouille(BOOL etat)
{
...
void CDlg1236::OnSelchangeModeTrafic() 
{
	int	iResult = ((CEquip1236*)equip)->ChangeModeTrafic(0,m_trafic.GetCurSel(),((CEquip1236*)equip)->Local() != 0);
	if(iResult<0);	// L'ancienne valeur sera rafraichie
}

void CDlg1236::OnModeDbl() 
{
	int	iResult = ((CEquip1236*)equip)->ChangeLocal(2,TRUE);
	if(iResult<0);	// L'ancienne valeur sera rafraichie
}

void CDlg1236::OnModeLoc() 
{
	int	iResult = ((CEquip1236*)equip)->ChangeLocal(0,TRUE);
	if(iResult<0);	// L'ancienne valeur sera rafraichie
}

void CDlg1236::OnModeRmt() 
{
	int	iResult = ((CEquip1236*)equip)->ChangeLocal(1,TRUE);
	if(iResult<0);	// L'ancienne valeur sera rafraichie
}


void CDlg1236::OnCompresseur() 
{
	int		iResult;

	int		val=((CButton*)GetDlgItem(F3_COMPRESSEUR))->GetCheck();
	iResult = ((CEquip1236*)equip)->ChangeCompresseur(val,((CEquip1236*)equip)->Local() != 0);
	if(iResult<0);	// L'ancienne valeur sera rafraichie
}

void CDlg1236::OnSelchangeModeFonct() 
{
	int	iResult = ((CEquip1236*)equip)->ChangeFonction(0,m_fonctionnement.GetCurSel(),((CEquip1236*)equip)->Local() != 0);
	if(iResult<0);	// L'ancienne valeur sera rafraichie
}

void CDlg1236::OnKillfocusCodeIncident() 
{
	int		iResult;

	UpdateData(TRUE);
	iResult = ((CEquip1236*)equip)->ChangeIncident(atoi(m_code_incident),((CEquip1236*)equip)->Local() != 0);
//	if(iResult<0);	// L'ancienne valeur sera rafraichie
}

void CDlg1236::OnKillfocusFrequence() 
{
	long	lResult;

	UpdateData(TRUE);
	lResult = ((CEquip1236*)equip)->ChangeFrequence(0,atoi(m_frequence),((CEquip1236*)equip)->Local() != 0);
	if(lResult<0);	// L'ancienne valeur sera rafraichie
}

void CDlg1236::OnAuto() 
{
	int		iResult;
	...
	if(m_choix_niveau.GetCurSel() != 0)
	{
		...
		iResult = ((CEquip1236*)equip)->ChangeAutoNiveau(val);
		if(iResult<0);	// L'ancienne valeur sera rafraichie
	}
	else
	{
		SendDlgItemMessage(F3_AUTO,BST_UNCHECKED,0,0);
		((CEquip1236*)equip)->ChangeAutoNiveau(0);
	}
}

void CDlg1236::OnKillfocusNiveau() 
{
	int		iResult;

	UpdateData(TRUE);
	iResult = ((CEquip1236*)equip)->ChangeNiveau(atoi(m_niveau),((CEquip1236*)equip)->Local() != 0);
//	if(iResult<0);	// L'ancienne valeur sera rafraichie
}

void CDlg1236::OnSelchangeChoixNiveau() 
{
	int	iResult = ((CEquip1236*)equip)->ChangeChoixNiveau(m_choix_niveau.GetCurSel(),((CEquip1236*)equip)->Local() != 0);
	if(iResult<0);	// L'ancienne valeur sera rafraichie
	/* Correction du 30/01/2008 */
	if(iResult == 0)
	{
...
void CDlg1236::OnKillfocusCanal() 
{
	int		iResult;

	UpdateData(TRUE);
	iResult = ((CEquip1236*)equip)->ChangeCanal(atoi(m_canal),((CEquip1236*)equip)->Local() != 0);
//	if(iResult<0);	// L'ancienne valeur sera rafraichie
}

void CDlg1236::OnMemoCanal() 
{
	int		iResult;

	int		num_canal = ((CEquip1236*)equip)->Canal();

	iResult = ((CEquip1236*)equip)->MemoriseCanal(num_canal,((CEquip1236*)equip)->Local() != 0);
//	if(iResult<0);	// L'ancienne valeur sera rafraichie
}

void CDlg1236::OnKillfocusCodeTest() 
{
	int		iResult;

	UpdateData(TRUE);
	iResult = ((CEquip1236*)equip)->ChangeTest(atoi(m_test),((CEquip1236*)equip)->Local() != 0);
//	if(iResult<0);	// L'ancienne valeur sera rafraichie
}

void CDlg1236::OnInfo() 
{
