...
VERSION :	1.0
ROLE :		Implémentation de la classe CDlg1237
***************************************************************************	*/
#include "stdafx.h"
#include "sicomex.h"
#include "Dlg1237.h"
...
CDlg1237::CDlg1237(CWnd* pParent /*=NULL*/)
	: CDlgEquip(CDlg1237::IDD, pParent)
{
	//{{AFX_DATA_INIT(CDlg1237)
	m_frequence = _T("");
	m_err_retard = _T("");
	m_canal = _T("");
	m_code_incident = _T("");
...
void CDlg1237::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CDlg1237)
...
BEGIN_MESSAGE_MAP(CDlg1237, CDialog)
	//{{AFX_MSG_MAP(CDlg1237)
	ON_WM_TIMER()
...
/////////////////////////////////////////////////////////////////////////////
// CDlg1237 message handlers

void CDlg1237::OnAide()
{
	int			iResult;
	CDlgAide	win;
...
void CDlg1237::GenerateurAuto()
{
	srand( (unsigned)time( NULL ) );

	if(((CEquip1237*)equip)->AutoNiveau())
		((CEquip1237*)equip)->ChangeNiveau(rand()%0xFF,FALSE);
}
// OnTimer : Rafraichissement de l'écran de controle **************************
// ****************************************************************************
void CDlg1237::OnTimer(UINT nIDEvent) 
{
	int			val;
	char		buf[10];
...
// Verrouille((((CEquip1237*)equip)->Local())!=1);
...
// Mode Trafic
	val = ((CEquip1237*)equip)->ModeTrafic(0);
	m_trafic.SetCurSel(val);

// Compresseur
	val = ((CEquip1237*)equip)->Compresseur();
	SendDlgItemMessage(F3_COMPRESSEUR,BM_SETCHECK,val,0);

// Mode de fonctionnement
	val = ((CEquip1237*)equip)->Fonction(0);
	m_fonctionnement.SetCurSel(val);

// Incident
	val = ((CEquip1237*)equip)->Incident();
...
// Frequence
	...
	{
		sprintf(buf,"%d",((CEquip1237*)equip)->Frequence(0));
		m_frequence = buf;
	}

// Niveau analogique
	val = ((CEquip1237*)equip)->AutoNiveau();
	SendDlgItemMessage(F3_AUTO,BM_SETCHECK,val,0);

	if(GetDlgItem(F3_NIVEAU) != GetFocus())
	{
		sprintf(buf,"%d",((CEquip1237*)equip)->Niveau());
		m_niveau = buf;
	}

	val = ((CEquip1237*)equip)->ChoixNiveau();
	m_choix_niveau.SetCurSel(val);

// Canal
	if(GetDlgItem(F3_CANAL) != GetFocus())
	{
		sprintf(buf,"%d",((CEquip1237*)equip)->Canal());
		m_canal = buf;
	}
...
// Test
	...
	{
		sprintf(buf,"%d",((CEquip1237*)equip)->Test());
		m_test = buf;
	}

// Presence Info
		m_info.Positionne_etat(((CEquip1237*)equip)->Info());
...
// ******************************************************************************
void CDlg1237::Verrouille(BOOL etat)
{
...
void CDlg1237::OnSelchangeModeTrafic() 
{
	int	iResult = ((CEquip1237*)equip)->ChangeModeTrafic(0,m_trafic.GetCurSel(),((CEquip1237*)equip)->Local() != 0);
	if(iResult<0);	// L'ancienne valeur sera rafraichie
}

void CDlg1237::OnModeDbl() 
{
	int	iResult = ((CEquip1237*)equip)->ChangeLocal(2,TRUE);
	if(iResult<0);	// L'ancienne valeur sera rafraichie
}

void CDlg1237::OnModeLoc() 
{
	int	iResult = ((CEquip1237*)equip)->ChangeLocal(0,TRUE);
	if(iResult<0);	// L'ancienne valeur sera rafraichie
}

void CDlg1237::OnModeRmt() 
{
	int	iResult = ((CEquip1237*)equip)->ChangeLocal(1,TRUE);
	if(iResult<0);	// L'ancienne valeur sera rafraichie
}


void CDlg1237::OnCompresseur() 
{
	int		iResult;

	int		val=((CButton*)GetDlgItem(F3_COMPRESSEUR))->GetCheck();
	iResult = ((CEquip1237*)equip)->ChangeCompresseur(val,((CEquip1237*)equip)->Local() != 0);
	if(iResult<0);	// L'ancienne valeur sera rafraichie
}

void CDlg1237::OnSelchangeModeFonct() 
{
	int	iResult = ((CEquip1237*)equip)->ChangeFonction(0,m_fonctionnement.GetCurSel(),((CEquip1237*)equip)->Local() != 0);
	if(iResult<0);	// L'ancienne valeur sera rafraichie
}

void CDlg1237::OnKillfocusCodeIncident() 
{
	int		iResult;

	UpdateData(TRUE);
	iResult = ((CEquip1237*)equip)->ChangeIncident(atoi(m_code_incident),((CEquip1237*)equip)->Local() != 0);
//	if(iResult<0);	// L'ancienne valeur sera rafraichie
}

void CDlg1237::OnKillfocusFrequence() 
{
	long	lResult;

	UpdateData(TRUE);
	lResult = ((CEquip1237*)equip)->ChangeFrequence(0,atoi(m_frequence),((CEquip1237*)equip)->Local() != 0);
	if(lResult<0);	// L'ancienne valeur sera rafraichie
}

void CDlg1237::OnAuto() 
{
	int		iResult;
	...
	if(m_choix_niveau.GetCurSel() != 0)
	{
		...
		iResult = ((CEquip1237*)equip)->ChangeAutoNiveau(val);
		if(iResult<0);	// L'ancienne valeur sera rafraichie
	}
	else
	{
		SendDlgItemMessage(F3_AUTO,BST_UNCHECKED,0,0);
		((CEquip1237*)equip)->ChangeAutoNiveau(0);
	}
}

void CDlg1237::OnKillfocusNiveau() 
{
	int		iResult;

	UpdateData(TRUE);
	iResult = ((CEquip1237*)equip)->ChangeNiveau(atoi(m_niveau),((CEquip1237*)equip)->Local() != 0);
//	if(iResult<0);	// L'ancienne valeur sera rafraichie
}

void CDlg1237::OnSelchangeChoixNiveau() 
{
	int	iResult = ((CEquip1237*)equip)->ChangeChoixNiveau(m_choix_niveau.GetCurSel(),((CEquip1237*)equip)->Local() != 0);
	if(iResult<0);	// L'ancienne valeur sera rafraichie
	/* Correction du 30/01/2008 */
	if(iResult == 0)
	{
...
void CDlg1237::OnKillfocusCanal() 
{
	int		iResult;

	UpdateData(TRUE);
	iResult = ((CEquip1237*)equip)->ChangeCanal(atoi(m_canal),((CEquip1237*)equip)->Local() != 0);
//	if(iResult<0);	// L'ancienne valeur sera rafraichie
}

void CDlg1237::OnMemoCanal() 
{
	int		iResult;

	int		num_canal = ((CEquip1237*)equip)->Canal();

	iResult = ((CEquip1237*)equip)->MemoriseCanal(num_canal,((CEquip1237*)equip)->Local() != 0);
//	if(iResult<0);	// L'ancienne valeur sera rafraichie
}

void CDlg1237::OnKillfocusCodeTest() 
{
	int		iResult;

	UpdateData(TRUE);
	iResult = ((CEquip1237*)equip)->ChangeTest(atoi(m_test),((CEquip1237*)equip)->Local() != 0);
//	if(iResult<0);	// L'ancienne valeur sera rafraichie
}

void CDlg1237::OnInfo() 
{
	int	iResult = m_info.Inverse_etat();
 	((CEquip1237*)equip)->ChangeInfo(iResult,TRUE);
}

void CDlg1237::OnCouplageEsclave() 
{
	int	iResult = ((CEquip1237*)equip)->ChangeCouplage(1,TRUE);
	if(iResult<0);	// L'ancienne valeur sera rafraichie
}

void CDlg1237::OnCouplageMaitre() 
{
