// TTabCtrl.cpp : implementation file
//
...
IMPLEMENT_DYNAMIC(CXTabCtrl, CTabCtrl)

CXTabCtrl::CXTabCtrl()
{
	m_bFirstBackColor=FALSE;
	m_crBackColor=::GetSysColor(COLOR_BTNFACE);
	m_bAutoPannelSize=FALSE;
	m_iSelectedTab = 0;
	m_ptTabs.x = 5;
	m_ptTabs.y = 28;
...
void CXTabCtrl::SetBackGroundColor(COLORREF cBackColor)
{
	m_crBackColor=cBackColor;
	m_bFirstBackColor=TRUE;

	Invalidate();
}
void CXTabCtrl::SetNoSelBackGroundColor(COLORREF cBackColor)
{
	//
	m_crNoBackColor=cBackColor;
...
//	ASSERT_VALID(pWnd);
	...
	if(pWnd) pWnd->SetWindowPos(NULL, m_ptTabs.x, m_ptTabs.y , 0, 0,
						SWP_FRAMECHANGED | SWP_NOSIZE | SWP_NOZORDER);

	if(pWnd) pWnd->ShowWindow(iIndex ? SW_HIDE : SW_SHOW);
...
// ------------------------------------------------------------------------------------------
void CXTabCtrl::ReSizeCurPannel()
{
	if(!m_bAutoPannelSize) return;
	...
	if (m_arrayStatusTab[iIndex])
	{
		...
		//Rect.top=RectOrg.top;
		//Rect.left=RectOrg.left;
		...
		GetItem(GetCurSel(), &item);
		pWnd = reinterpret_cast<CWnd*> (item.lParam);
		pWnd->MoveWindow(Rect.left+1,Rect.top+(m_ptTabs.y-6),Rect.Width()-4/*(m_ptTabs.x*2)*/,Rect.Height()-(m_ptTabs.y-3));
	}
}
// ------------------------------------------------------------------------------------------
...
//-----------------------------------------------------
CDialog *CXTabCtrl::GetDialogCtrl(int nIndex)
{
...
//-----------------------------------------------------
/*HBRUSH CXTabCtrl::CtlColor(CDC* pDC, UINT nCtlColor)
{
